...
public class PullMessageProcessor implements NettyRequestProcessor {
    ...
    /* CODE_MARK [pull-message] 处理消息拉取命令，
     * 如果是 Client 发来的命令， brokerAllowSuspend 为 true，
     * 拉不到消息会将请求放到 PullRequestHoldService，待会再查，同时不给 Client Response
     *
     * PullRequestHoldService 发现有新消息或过了等待时间后，就会用原请求再一次调用 processRequest
     * 这时 brokerAllowSuspend 为 false
     * 如果还是拉不到消息就不再将请求放到 PullRequestHoldService，同时返回 PULL_NOT_FOUND 给 client
    **/
    private RemotingCommand processRequest(final Channel channel, RemotingCommand request, boolean brokerAllowSuspend)
        ...
        /* CODE_MARK [pull-message] 从 store 中获取指定 consume queue 中的消息 */
        ...
        if (getMessageResult != null) {
            ...
            /* CODE_MARK [pull-message] 拉取消息时，如果 master 太忙，让 client 转到 slave */
